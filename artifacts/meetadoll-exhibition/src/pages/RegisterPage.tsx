  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const result = await apiRequest<{ requiresOtp: boolean; userId: string; email: string }>(
        ...
        {
          method: "POST",
          body: { name, email, password, phone: phone || undefined, vendor_category: vendorCategory },
          skipAuthRetry: true,
        }
      );
      ...
      <div className="w-full max-w-sm">
        {step === "form" ? (
          <>
            ...
            <form onSubmit={handleRegister} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                ...
                <Input id="name" required value={name} onChange={(e) => setName(e.target.value)} placeholder="Amira Bello" />
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                >
                  ...
                  <option value="fashion">Fashion &amp; Others</option>
                </select>
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="relative">
                  <Input id="password" type={showPassword ? "text" : "password"} autoComplete="new-password" required minLength={8} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min. 8 characters" className="pr-10" />
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="relative">
                  <Input id="confirm" type={showConfirm ? "text" : "password"} autoComplete="new-password" required value={confirm} onChange={(e) => setConfirm(e.target.value)} placeholder="••••••••" className="pr-10" />
              ...
              <label className="flex items-start gap-3 cursor-pointer group">
                <input type="checkbox" checked={agreed} onChange={(e) => setAgreed(e.target.checked)} className="mt-0.5 h-4 w-4 rounded border-border accent-primary cursor-pointer" />
          ...
          <>
            ...
            <p className="text-muted-foreground text-sm text-center mb-2">We sent a 6-digit code to</p>
            ...
            <form onSubmit={handleVerifyOtp} className="flex flex-col gap-6">
              <div className="flex flex-col items-center gap-3">
                ...
                {otpCountdown === 0 && <p className="text-xs text-destructive">Code expired. Request a new one below.</p>}
              </div>

              {error && <p className="text-sm text-destructive text-center">{error}</p>}
              {info && <p className="text-sm text-green-600 text-center">{info}</p>}

              <Button type="submit" className="rounded-full" disabled={loading || otp.length !== 6 || otpCountdown === 0}>
            ...
            <div className="mt-5 text-center">
              <p className="text-sm text-muted-foreground">
                Didn&apos;t receive it?{" "}
                <button type="button" onClick={handleResend} disabled={resendCooldown > 0 || loading} className="text-primary hover:underline font-medium disabled:opacity-40 disabled:cursor-not-allowed">
                  {resendCooldown > 0 ? `Resend in ${resendCooldown}s` : "Resend code"}
                </button>
              </p>
              <button type="button" onClick={() => { setStep("form"); setError(""); setOtp(""); if (otpTimer.current) clearInterval(otpTimer.current); }} className="mt-2 text-xs text-muted-foreground hover:underline">
