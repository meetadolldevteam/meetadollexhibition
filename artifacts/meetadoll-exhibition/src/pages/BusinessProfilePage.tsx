...
export default function BusinessProfilePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Refresh user so business_profile_complete flips to true
      try {
        await refreshUser();
      } catch {
        // Even if refresh fails, profile was saved — proceed to picker
      }

      // Always open the stall picker after a successful save
      setPickerOpen(true);
    } catch (err) {
  ...
  return (
    <>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="business_phone">
                Business phone <span className="text-muted-foreground text-xs">(optional)</span>
              </Label>
              <Input
                id="business_phone"
                type="tel"
            ...
            <div className="flex flex-col gap-1.5">
              <Label>
                Business logo <span className="text-muted-foreground text-xs">(optional)</span>
              </Label>
              {logoPreview ? (
                <div
      ...
      {/* Stall picker opens immediately after successful save */}
      <StallPickerModal
        open={pickerOpen}
        onOpenChange={(open) => {
          setPickerOpen(open);
